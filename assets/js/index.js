document.addEventListener("DOMContentLoaded", () => {
  ...
  if (container) {
    ...
    function createStethoscope(scale) {
      ...
      const upperTorusGeo = new THREE.TorusGeometry(
        0.4 * scale,
        tr,
        16,
        32,
      ...
      const lowerLeftVertGeo = new THREE.CylinderGeometry(
        tr,
        tr,
        0.8 * scale,
        16,
      ...
      const lowerTorusGeo = new THREE.TorusGeometry(
        0.4 * scale,
        tr,
        16,
        32,
      ...
      const lowerRightVertGeo = new THREE.CylinderGeometry(
        tr,
        tr,
        1.2 * scale,
        16,
      ...
      const chestGeo = new THREE.TorusGeometry(
        0.18 * scale,
        0.07 * scale,
        16,
        32,
    ...
    const elementConfigs = [
      ...
      {
        create: () => createMedicalCross(3.0),
        pos: [-20, 10, -10],
        drift: [0.1, 0.07, 0.04],
        rot: [0.002, 0.004, 0.005],
        delay: 0.2,
      },
      {
        create: () => createCapsule(2.2),
        pos: [-16, -8, -12],
        drift: [0.08, 0.12, 0.05],
        rot: [0.003, 0.002, 0.005],
        delay: 0.6,
      },

      // Center — bridging the two sides
      {
        create: () => createTestTube(1.8),
        pos: [-3, 13, -16],
        drift: [0.06, 0.05, 0.07],
        rot: [0.003, 0.003, 0.002],
        delay: 0.4,
      },
      {
        create: () => createStethoscope(2.0),
        pos: [4, -1, -14],
        drift: [0.07, 0.09, 0.04],
        rot: [0.002, 0.005, 0.003],
        delay: 0.8,
      },

      // Right side (behind phone mockup) — slightly larger
      {
        create: () => createStethoscope(2.4),
        pos: [20, 8, -8],
        drift: [0.09, 0.06, 0.05],
        rot: [0.003, 0.005, 0.002],
        delay: 0.3,
      },
      {
        create: () => createCapsule(2.0),
        pos: [34, -6, -10],
        drift: [0.07, 0.1, 0.06],
        rot: [0.004, 0.003, 0.005],
        delay: 0.7,
      },
      {
        create: () => createMedicalCross(1.6),
        pos: [34, 7, -14],
        drift: [0.1, 0.08, 0.04],
        rot: [0.003, 0.004, 0.003],
        delay: 1.0,
      },
      {
        create: () => createTestTube(2.0),
        pos: [22, 14, -12],
        drift: [0.05, 0.07, 0.08],
    ...
    function animate() {
      ...
      floatingElements.forEach((el) => {
        ...
        el.mesh.position.y =
          el.basePos.y + Math.sin(t * el.drift[1] + 1.5) * 2.5;
        el.mesh.position.z =
          el.basePos.z + Math.sin(t * el.drift[2] + 0.8) * 1.0;
        ...
        el.mesh.rotation.z =
          el.baseRot.z + Math.cos(t * el.rot[2] * 150) * 0.15;
      });
  ...
  seqTl.to(
    ".download-label, .app-download-buttons",
    {
      opacity: 1,
      y: 0,
      duration: 1.0,
  ...
  const n1X = 0,
    n1Y = -r; // Top
  const n2X = r * Math.cos(Math.PI / 6),
    n2Y = r * Math.sin(Math.PI / 6); // Bottom-right
  const n3X = -r * Math.cos(Math.PI / 6),
    n3Y = r * Math.sin(Math.PI / 6); // Bottom-left
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    crossGroup.add(meshV);
    crossGroup.add(meshH);
    ...
    const ring2 = new THREE.Mesh(ringGeo, crystalMat);
    ring2.rotation.y = Math.PI / 3;
    ring2.rotation.x = Math.PI / 4;
    const ring3 = new THREE.Mesh(ringGeo, crystalMat);
    ...
    const rimLight1 = new THREE.PointLight(0x06b6d4, 10, 50);
    rimLight1.position.set(10, 5, -10);
    scene.add(rimLight1);
    const rimLight2 = new THREE.PointLight(0xc026d3, 10, 50);
    ...
    courseItems.forEach((item) => {
      item.addEventListener("mouseenter", () => {
        ...
        if (currentObject !== targetObject) {
          ...
          gsap.to(rimLight1.color, {
            r: ((colors.c1 >> 16) & 255) / 255,
            g: ((colors.c1 >> 8) & 255) / 255,
            b: (colors.c1 & 255) / 255,
            duration: 1,
          });
          gsap.to(rimLight2.color, {
            r: ((colors.c2 >> 16) & 255) / 255,
            g: ((colors.c2 >> 8) & 255) / 255,
            b: (colors.c2 & 255) / 255,
            duration: 1,
          });
  ...
  aboutTl
    ...
    .from(
      [".orbital-ring", ".year-badge", ".rotating-badge"],
      {
        scale: 0.8,
        opacity: 0,
        duration: 0.6,
  ...
  if (gyroContainer) {
    ...
    const gyroCamera = new THREE.PerspectiveCamera(
      50,
      window.innerWidth / window.innerHeight,
      0.1,
      1000,
    ...
    const gyroRenderer = new THREE.WebGLRenderer({
      alpha: true,
      antialias: true,
    ...
    const envL1 = new THREE.PointLight(0xd4b3ff, 15, 100);
    envL1.position.set(10, 10, 10);
    const envL2 = new THREE.PointLight(0x7dd3fc, 15, 100);
    envL2.position.set(-10, -10, 10);
    const envL3 = new THREE.PointLight(0xffffff, 10, 100);
    envL3.position.set(0, 0, -10);
    ...
    const meshRing1 = new THREE.Mesh(
      new THREE.TorusGeometry(7, 0.25, 24, 120),
      ringMat,
    );
    const meshRing2 = new THREE.Mesh(
      new THREE.TorusGeometry(5.5, 0.3, 24, 120),
      ringMat,
    );
    const meshRing3 = new THREE.Mesh(
      new THREE.TorusGeometry(4, 0.35, 24, 120),
      ringMatAccent,
    );
    ...
    const outerRing1 = new THREE.Mesh(
      new THREE.TorusGeometry(9, 0.08, 16, 150),
      outerRingMat,
    );
    const outerRing2 = new THREE.Mesh(
      new THREE.TorusGeometry(10, 0.06, 16, 150),
      outerRingMat,
    );
    ...
    const gLight1 = new THREE.PointLight(0x06b6d4, 6, 50);
    gLight1.position.set(12, 8, 12);
    const gLight2 = new THREE.PointLight(0xc026d3, 6, 50);
    gLight2.position.set(-12, -8, 10);
    const gLight3 = new THREE.PointLight(0x7c3aed, 4, 40);
    gLight3.position.set(0, 12, -10);
    const gLight4 = new THREE.PointLight(0xfbbf24, 2, 40);
    gLight4.position.set(8, -10, 8);
  ...
  function initFacultyDots() {
    ...
    ctaWrap.innerHTML =
      '<a href="/faculties.html" class="btn-meet-mentors"><i class="fa-solid fa-users"></i> Meet All Mentors</a>';
  ...
  // Header reveal
  gsap.from(
    ".impact-header .frost-pill-dark, .impact-header .impact-title, .impact-header .impact-subtitle",
    {
      scrollTrigger: {
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      ...
      document
        .querySelectorAll("#itile-success-trigger .ibar-fill-white")
        .forEach((bar) => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        ...
        gsap.to(countObj, {
          ...
          ease: "power2.out",
          onUpdate: () => {
            counter.innerText = Math.floor(countObj.val).toLocaleString(
              "en-IN",
            );
          },
        });
