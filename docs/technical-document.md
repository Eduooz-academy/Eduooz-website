...
- 58 individual exam-detail pages (one per recruitment exam) sharing one rich template: eligibility checker, syllabus, previous-question-paper PDF viewer, mock-test CTA, FAQ.
...
Documenting every one of the 84 HTML files individually would mean repeating the same template ~58 times (the exam pages are generated from one template — see [§4](#4-html-documentation)). Instead, this section documents every **unique** file (all CSS, all JS, all components, all config) in full, and gives the exam-page template plus a complete inventory table once.
...
| `practice-test.js`     | 1,035 (incl. bundled `question-bank.js` loader) | Course pages with a mock-test panel             | Category → Topic → Test → Question quiz engine; fetches question banks                                                                                                                                               |
...
Every file under `courses/{nursing,pharmacy,mlt}/{central,kerala,gcc}/*.html` (57 files) is built from the same template. Rather than documenting each one, here is the template contract every exam page follows — **this is what you copy when adding a new exam page** (see [§23](#23-modification-guide)):
...
1. `#header-container` / `#footer-container` / `#chat-container` (empty, filled by `components.js`) — note `#enquiry-form-container` is _not_ a separate container on exam pages the way it is on marketing pages; the lead form appears inline within gated flows (syllabus/QP-download lead modals) instead.
...
Documented as a template in [§3.3](#33-individual-exam-page--the-shared-template) with the full 58-page inventory in [§3.4](#34-complete-course-page-inventory).
...
`assets/css/global-heading-system.css` is the canonical shared token file (its own header comment says so: _"Canonical typography, pill labels, gradient text, and spacing system. Load this BEFORE any page-specific stylesheet."_). Its `:root` block:
...
**⚠️ Duplicate/competing `:root` blocks — add new variables here, nowhere else.** Both `about.css` and `legal-shared.css` define their _own_ additional `:root` block with the _same variable names_ but different values:
...
| `--glass-border` | `rgba(255,255,255,0.12)`      | _(not redefined)_                                    | `#e2e8f0`                     |
| `--font-serif`   | `"Cormorant Garamond", serif` | _(commented out — falls through to the global file)_ | `"Cormorant Garamond", serif` |
...
### 6.4 `course-landing.js` (6,641 lines — the largest script)
...
- **JS:** none inline — `forms.js` handles submission via event delegation on `document`, which is why this form works correctly even though it's injected asynchronously _after_ `forms.js` has already run.
...
- **Sitemap:** `sitemap.xml` — 73 `<url>` entries, `<loc>` + `<lastmod>` only (no `priority`/`changefreq`). Covers all top-level pages, the 3 category hubs, and every one of the 57 individual exam pages plus `german-language.html`. Update whenever a page is added/removed/renamed; only bump `<lastmod>` when content meaningfully changes.
...
4. If it's a new _category_ of page (not just a new exam), also update `llms.txt`'s course-category section.
...
- **`course-landing.css` (16,427 lines) and `course-landing.js` (6,641 lines)** are both loaded in full on every single exam page even though any given page only exercises a fraction of their selectors/functions — there is no per-page code-splitting since there's no build step to do it with.
...
The `CNAME` file (containing `eduooz.com`) indicates this repo is _intended_ to deploy via **GitHub Pages** from this repository. **As of the last documented SEO audit (2026-07-09), the live domain `https://www.eduooz.com/` was still serving an older WordPress site (Elementor + Yoast SEO, hosted on Hostinger) — this static repo had not yet gone live.** Confirm current production status before assuming this repo is what visitors currently see.
...
_(Describes the conventions actually observed in the codebase — not all files follow every rule perfectly, but these are the dominant patterns to match.)_
